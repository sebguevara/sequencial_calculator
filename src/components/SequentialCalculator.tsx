...
import type { ArbitrageResult, Match, CombinedBet } from "@/interfaces/betting.types";
...
const SequentialCalculator: React.FC = () => {
  ...
  const computeResult = useCallback((): ArbitrageResult | null => {
    ...
    const combinedBet: CombinedBet = { stake: combinedStake, matches, combinedOdds };
  ...
  const canClear     = combinedStake > 0 || matches.length > 0 || counterOdds.length > 0;
  ...
  const renderMobileButtons = () =>
    isMobile && (
      <div className="flex flex-col gap-2 w-full">
        <Button className="cursor-pointer" onClick={handleCalculate} disabled={!canCalculate}>
    ...
    <div className="flex flex-col items-center my-4 mx-4 max-w-full overflow-x-hidden">
      <div className="flex lg:px-4 flex-col md:flex-row w-full justify-center items-center mb-6">
        ...
        {!isMobile && (
          <div className="flex gap-2 w-full justify-end mt-4 md:mt-0">
            <Button className="cursor-pointer" variant="outline" onClick={clearAll} disabled={!canClear}>
              Limpiar
            </Button>
            <Button className="cursor-pointer" onClick={handleCalculate} disabled={!canCalculate}>
      ...
      <div className="space-y-6 w-full ">
        <div className="flex flex-col md:flex-row md:justify-center gap-6">
          <CombinedBetInput
            stake={combinedStake}
            onStakeChange={(v) => { setCombinedStake(v); setResult(null); }}
            matches={matches}
            onMatchesChange={(m) => { setMatches(m); setResult(null); }}
            combinedOdds={combinedOdds}
          />
          <CounterBetsInput
            matches={matches}
            counterOdds={counterOdds}
            onCounterOddsChange={(o) => { setCounterOdds(o); setResult(null); }}
            result={result}
          />
        </div>
